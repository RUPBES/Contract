...
namespace BusinessLayer.Services
{
    internal class SWCostService : ISWCostService
    {
        ...
        public SWCostService(IContractUoW database, IMapper mapper, IContractsLogger logger, IContractArchiveUoW databaseArch)
        {
            _database = database;
            _mapper = mapper;
            _logger = logger;
        ...
        public int? Create(SWCostDTO item)
        {
            if (item is not null)
            {
                if (_database.SWCosts.GetById(item.Id) is null)
                {
                    ...
                    _database.Save();
                    _logger.WriteLog(LogLevel.Information, $"create scope work costs, ID={model.Id}", typeof(SWCostService).Name, MethodBase.GetCurrentMethod().Name);
